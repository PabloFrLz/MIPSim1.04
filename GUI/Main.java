...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) throws IOException{
        ...
        loadButton.setOnAction(event -> {
            Arrays.fill(mips.memoria.enderecamento, (byte) 0); //remove todas as demais instruções na memoria do MIPS preenchendo com 0's cada byte.
            ...
            error = mips.memoria.instruction_memory.carrega_instrucoes_na_memoria("code.asm");

            if(error == 1){
                return;// interrompe a execução do botão LOAD nesse ponto quando o usuario inserir uma instrução inválida.
            }

            //carrega o registrador Global Pointer com o endereço ao centro do segmento GLOBAL DATA. //$gp = ((0x7FFB0 - 0x7FF38) / 2) + 0x7FF38 = 0x7FF74
            mips.registerFile.setRegistradores(28, ((Memory.endereco_final_GLOBAL_DATA - Memory.endereco_inicial_GLOBAL_DATA) / 2) + Memory.endereco_inicial_GLOBAL_DATA);
            infoPath.atualizaInfo(122);
            
            //carrega o registrador Stack Pointer com o endereço final do segmento DYNAMIC DATA. 
            mips.registerFile.setRegistradores(29, Memory.endereco_final_DYNAMIC_DATA);
            infoPath.atualizaInfo(122); 

            //por padrão, o MIPS também carrega a instrução "jal endereço_inical_TEXT". Mas para essa implementação, vamos desconsiderar e supor que essa instrução já foi executada.
